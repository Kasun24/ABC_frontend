...
<template>
  <v-menu :close-on-content-click="false">
    ...
    <v-card width="420">
      <v-card-title class="d-flex justify-space-between">
        <div>
          <span class="text-subtitle-2 font-weight-bold">{{ $t('t-my-cart') }}</span>
          <v-badge inline :content="cartItemsVar.length" />
        </div>
        <v-btn variant="text" color="primary"> {{ $t('t-view-all') }} </v-btn>
      </v-card-title>
      ...
      <v-card-text data-simplebar style="max-height: 300px" class="px-6">
        ...
        >
          ...
          <h4 class="mb-5">{{ $t('t-your-cart-is-empty') }}</h4>
          <v-btn color="success" class="mb-3" elevation="0"> {{ $t('t-shop-now') }}</v-btn>
        </div>
        <div v-else>
          <v-row
            no-gutters
          ...
          <h4 class="d-flex justify-space-between py-3">
            <div>{{ $t('t-sub-total') }} :</div>
            <div>$1306.52</div>
          </h4>
          <h4 class="d-flex justify-space-between py-3">
            <div>{{ $t('t-shipping-charge') }} :</div>
            <div>$65.00</div>
          </h4>
          <h4 class="d-flex justify-space-between py-3">
            <div>{{ $t('t-estimated-tax') }} (12.5%) :</div>
      ...
      >
        ...
        <v-btn block color="info" href="/cashier/checkout" variant="elevated">
          {{ $t('t-checkout') }}
        </v-btn>
      </v-card-actions>
    </v-card>
  ...
  >
    ...
    <v-card-text class="text-center ma-md-5">
      ...
      <div class="mt-4">
        <h4>{{ $t('t-are-you-sure') }} ?</h4>
        <p class="text-muted mx-4 mb-0">
          {{ $t('t-confirm-remove-product') }} ?
        </p>
      </div>
    </v-card-text>
  </ConfirmationDialog>
