...
<template>
  <v-menu :close-on-content-click="false">
    ...
    <v-card width="320" style="overflow: hidden">
      <v-card-title>
        <h5>
          <div class="text-subtitle-2 font-weight-medium">{{ $t('t-notifications') }}</div>
        ...
        <span class="font-weight-regular text-muted">
          {{ $t('t-you-have') }}
          <b> {{ unreadMessages.length }} </b> {{ $t('t-unread-messages') }}
        </span>
      ...
      <v-card-text data-simplebar style="max-height: 300px" class="px-2">
        <h5 class="text-muted my-2">{{ $t('t-new') }}</h5>
        <div
        ...
        <h5 class="text-muted my-2 text-uppercase">{{ $t('t-read-before') }}</h5>
      ...
      >
      {{ $t('t-select') }} {{ selectedMsg }} {{ $t('t-result') }}
        <v-btn variant="text" color="danger" @click="onRemove"> {{ $t('t-remove') }} </v-btn>
      </v-card-actions>
    </v-card>
  </v-menu>
</template>
